...
import { View, Text, TouchableOpacity, StyleSheet, Modal, ScrollView, Image, Platform } from 'react-native'
...
export default function ProfileModal({
  user, visible, onClose, onChat, isFavorite, onToggleFavorite, onBlock, onReport
}: {
  user: User | null
  visible: boolean
  onClose: () => void
  onChat: (userId: string, user: User) => void
  isFavorite: boolean
      ...
      <View style={s.container}>
        ...
        <ScrollView contentContainerStyle={s.content}>
          <View style={s.photoSection}>
            {user.avatar_url
              ? <TouchableOpacity onPress={() => setZoomPhoto(user.avatar_url || null)}>
                  <Image source={{ uri: user.avatar_url }} style={s.photo} />
                </TouchableOpacity>
              : <View style={s.photoPlaceholder}>
                  <Text style={s.photoPlaceholderText}>{user.display_name?.[0] || '?'}</Text>
                </View>}
            ...
            <View style={s.actions}>
              ...
              >
                <Text style={s.actionIcon}>star</Text>
              </TouchableOpacity>
              <TouchableOpacity style={s.actionBtn} onPress={() => onBlock(user.id, user.display_name)}>
                <Text style={s.actionIcon}>block</Text>
              </TouchableOpacity>
              {onReport && (
                <TouchableOpacity style={s.actionBtn} onPress={() => onReport(user.id, user.display_name)}>
                  <Text style={s.actionIcon}>report</Text>
                </TouchableOpacity>
              )}
              <TouchableOpacity
                style={[s.actionBtn, s.chatBtn]}
                onPress={() => onChat(user.id, user)}
              >
                <Text style={s.actionIcon}>chat</Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          {photos.length > 1 && (
            ...
            >
              {photos.map(p => (
                <TouchableOpacity key={p.id} onPress={() => setZoomPhoto(p.photo_url)}>
                  <Image source={{ uri: p.photo_url }} style={s.stripPhoto} />
                </TouchableOpacity>
              ))}
            </ScrollView>
          )}
          ...
          {(user.looking_for || user.relationship_type) && (
            <View style={s.card}>
              {user.looking_for && (
                <View style={s.infoRow}>
                  <Text style={s.infoLabel}>LOOKING FOR</Text>
                  <Text style={s.infoValue}>{user.looking_for}</Text>
                </View>
              )}
              {user.relationship_type && (
                <View style={s.infoRow}>
                  <Text style={s.infoLabel}>RELATIONSHIP TYPE</Text>
                  <Text style={s.infoValue}>{user.relationship_type}</Text>
                </View>
              )}
            </View>
          )}
            ...
            <View style={s.card}>
              ...
              <View style={s.prefTags}>
                {user.preferences.map(p => (
                  <View key={p} style={s.prefTag}>
                    <Text style={s.prefTagText}>{p}</Text>
                  </View>
                ))}
              </View>
            </View>
...
const s = StyleSheet.create({
  ...
  header: { paddingTop: Platform.OS === 'android' ? 40 : 20, paddingHorizontal: 20, paddingBottom: 10, flexDirection: 'row', justifyContent: 'flex-end' },
  ...
  actionIcon: { fontSize: 14, color: '#ffffff', fontWeight: '600' },
  info: { padding: 20 },
