...
if(uploadError){Alert.alert('Upload failed',uploadError.message);setUploadingPhoto(false);return}
...
const{data:{user}}=await supabase.auth.getUser()
if(!user)return
const{error}=await supabase.from('users').update({display_name:editData.display_name,bio:editData.bio,location:editData.location,looking_for:editData.looking_for,relationship_type:editData.relationship_type,preferences:editData.preferences||[]}).eq('id',user.id)
...
Alert.alert('Sign Out','Are you sure?',[{text:'Cancel',style:'cancel'},{text:'Sign Out',style:'destructive',onPress:()=>supabase.auth.signOut()}])
}
...
const editButton=(<TouchableOpacity onPress={()=>editing?handleSave():setEditing(true)} style={s.editButton}><Text style={s.editButtonText}>{editing?(saving?'Saving...':'Save'):'Edit'}</Text></TouchableOpacity>)
return(
...
{editing?<TextInput style={s.editNameInput} value={editData.display_name} onChangeText={v=>setEditData(prev=>({...prev,display_name:v}))} placeholder="Display name" placeholderTextColor="rgba(255,255,255,0.6)"/>
...
{editing?<TextInput style={[s.editInput,s.bioInput]} value={editData.bio} onChangeText={v=>setEditData(prev=>({...prev,bio:v}))} placeholder="Tell people about yourself..." placeholderTextColor="rgba(255,255,255,0.6)" multiline/>:<Text style={s.infoValue}>{profile?.bio||'No bio yet'}</Text>}
<Text style={s.infoLabel}>Age</Text>
<Text style={s.infoValue}>{profile?.age}</Text>
<Text style={s.infoLabel}>Location</Text>
{editing?<TextInput style={s.editInput} value={editData.location} onChangeText={v=>setEditData(prev=>({...prev,location:v}))} placeholder="e.g. New York, NY" placeholderTextColor="rgba(255,255,255,0.6)"/>:<Text style={s.infoValue}>{profile?.location||'Not set'}</Text>}
<Text style={s.infoLabel}>Looking For</Text>
{editing?<TextInput style={s.editInput} value={editData.looking_for} onChangeText={v=>setEditData(prev=>({...prev,looking_for:v}))} placeholder="e.g. Friends, Dating, Relationship" placeholderTextColor="rgba(255,255,255,0.6)"/>:<Text style={s.infoValue}>{profile?.looking_for||'Not set'}</Text>}
<Text style={s.infoLabel}>Relationship Type</Text>
{editing?<TextInput style={s.editInput} value={editData.relationship_type} onChangeText={v=>setEditData(prev=>({...prev,relationship_type:v}))} placeholder="e.g. Monogamous, Open, Casual" placeholderTextColor="rgba(255,255,255,0.6)"/>:<Text style={s.infoValue}>{profile?.relationship_type||'Not set'}</Text>}
</View>
<View style={s.infoCard}>
...
return(<TouchableOpacity key={pref} style={s.prefItem} onPress={()=>editing&&togglePreference(pref)} activeOpacity={editing?0.7:1}>
<View style={[s.prefCheckbox,isSelected&&s.prefCheckboxSelected]}>{isSelected&&<Text style={s.prefCheckmark}>✓</Text>}</View>
...
{editing&&<TouchableOpacity style={s.cancelButton} onPress={()=>{setEditing(false);setEditData(profile||{})}}><Text style={s.cancelButtonText}>Cancel</Text></TouchableOpacity>}
<TouchableOpacity style={s.blockedButton} onPress={()=>setShowBlocked(true)}><Text style={s.blockedButtonText}>🚫 Manage Blocked Users</Text></TouchableOpacity>
<TouchableOpacity style={s.signOutButton} onPress={handleSignOut}><Text style={s.signOutText}>Sign Out</Text></TouchableOpacity>
</ScrollView>
)
}
...
photoHint:{color:'rgba(255,255,255,0.5)',fontSize:11,marginTop:4,marginBottom:8},
editNameInput:{backgroundColor:'rgba(255,255,255,0.15)',borderRadius:10,padding:10,color:'#ffffff',fontSize:20,fontWeight:'700',textAlign:'center',borderWidth:1,borderColor:'rgba(255,255,255,0.3)',marginBottom:4,width:'80%'},
displayName:{color:'#ffffff',fontSize:22,fontWeight:'700',marginBottom:4,marginTop:8},
...
statNumber:{fontSize:24,fontWeight:'bold',color:'#F5A623'},
...
prefSubtitle:{fontSize:12,color:'rgba(255,255,255,0.6)',marginBottom:12},
infoLabel:{fontSize:12,color:'rgba(255,255,255,0.6)',fontWeight:'600',marginBottom:4,textTransform:'uppercase'},
infoValue:{fontSize:15,color:'#ffffff',marginBottom:16,lineHeight:22},
editInput:{backgroundColor:'rgba(255,255,255,0.15)',borderRadius:10,padding:12,color:'#ffffff',fontSize:15,marginBottom:16,borderWidth:1,borderColor:'rgba(255,255,255,0.3)'},
bioInput:{height:80,textAlignVertical:'top'},
prefItem:{flexDirection:'row',alignItems:'center',paddingVertical:10,borderBottomWidth:1,borderBottomColor:'rgba(255,255,255,0.1)'},
...
cancelButton:{backgroundColor:'rgba(255,255,255,0.15)',borderRadius:12,padding:16,alignItems:'center',marginHorizontal:24,marginBottom:12,borderWidth:1,borderColor:'rgba(255,255,255,0.2)'},
cancelButtonText:{color:'rgba(255,255,255,0.8)',fontSize:16,fontWeight:'600'},
blockedButton:{backgroundColor:'rgba(255,255,255,0.15)',borderRadius:12,padding:16,alignItems:'center',marginHorizontal:24,marginBottom:12,borderWidth:1,borderColor:'rgba(255,255,255,0.2)'},
blockedButtonText:{color:'rgba(255,255,255,0.8)',fontSize:16,fontWeight:'600'},
signOutButton:{backgroundColor:'rgba(255,255,255,0.15)',borderRadius:12,padding:16,alignItems:'center',marginHorizontal:24,borderWidth:1,borderColor:'#FF8C00'},
